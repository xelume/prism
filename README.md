...
- 只支持当前检查过的官方 macOS ChatGPT `26.825.51511`、默认目录、文件认证及完整 ChatGPT 令牌格式。历史 `Codex Auth` 钥匙串条目会被忽略且不会被修改；显式 `keyring`、`auto`、`ephemeral`、Secrets、版本更新、未知认证格式、自定义登录策略或不安全文件权限时拒绝操作，绝不自动修改安全策略。
