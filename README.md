...
本机 Mac 菜单栏工具：可在不中断当前 ChatGPT、Codex CLI 或 IDE 任务的情况下保存和添加账号；切换默认认证时才要求相关进程停止，并在桌面模式下重开 ChatGPT。

仅处理默认 `~/.codex/auth.json`。**不切换整个环境，不修改设置、插件、工作区或任务文件。** Codex CLI 和 IDE 扩展使用同一文件认证时也会换号。这不是 OpenAI 官方支持的账号切换接口。

## 使用

1. 打开下载的 DMG，将 `Prism.app` 拖入 Applications，推出磁盘映像后从“应用程序”启动。菜单栏出现星烁图标，点击即可展开账号菜单。启动后自动读取已保存账号及默认文件认证，并查询额度；首次使用或重新构建后可能需要点击“授权并重试”允许本工具访问账号备份钥匙串。后台读取不会主动弹出钥匙串授权框。
2. 选择“保存当前账号”。工具不退出 ChatGPT，直接以认证中的登录邮箱作为账号名称，将认证备份到本机钥匙串；认证中没有可用邮箱时使用编号名称。
3. 选择“添加账号”。工具使用 ChatGPT.app 内置或独立安装的官方 Codex CLI，在隔离的临时 `CODEX_HOME` 中打开浏览器登录；当前默认认证和运行中的任务不变。等待窗口可随时取消，3 分钟没有完成会自动超时。登录成功后自动校验认证、以邮箱命名并保存到钥匙串，临时目录随后删除。
4. 已保存账号直接显示在菜单中，当前账号以勾选和“当前”标注。点击其他账号，确认“是否切换到「账号名」？”后才执行切换。每次切换自动更新离开账号的最新认证，不用手动反复保存。后台账号显示“登录失效”时，点击该账号可通过相同的隔离登录流程重新认证；登录了其他账号时不会覆盖原备份。
...
添加或重新登录账号时，浏览器可能记住上一个账号，请核对实际选择。取消会立即结束 Prism 启动的隔离登录进程；如果进程拒绝正常退出，只强制结束该临时登录进程。取消、超时或认证格式不受支持时不会修改当前登录或账号备份。官方登录组件本身不提供后台账号的常驻刷新；当前账号由正在使用它的 Codex 自动续期，后台账号失效后需点击重新登录。

## 所有账号额度与后台刷新

- 菜单每个账号显示完整名称及两行额度：**5h 剩余百分比和距离重置的小时／分钟数**、**Week 剩余百分比和本机时区的具体重置日期、时间**。当前账号置顶，其他账号保持保存顺序。移除鼠标悬停提示，不显示最后刷新时间；登录失效、查询失败等异常才标注状态。时间已到但没有获得新的额度数据时显示等待重置确认，不自动推断额度恢复。切换确认框显示完整名称，VoiceOver 可读取完整名称及额度。当前已登录但尚未保存的账号也显示额度，并标记“未保存账号”；保存后才能在列表中切回它。
...
- **Prism 不会自动刷新 access token 或轮换 refresh token。** 后台账号的令牌失效后可点击账号并通过官方 Codex 登录组件重新认证；账号服务端撤销登录、权限不足、网络故障或接口变化也可能导致无法查询。401 标记登录失效，403 标记拒绝访问，不能把所有失败都当成额度耗尽。
...
- 官方当前认证采用默认 `~/.codex/auth.json` 文件存储；替换文件使用 `0600` 权限和同目录原子重命名。其他配置不变。
- 每次替换前保存当前最新认证；备份失败不替换文件。启动失败时，仅在相关进程均停止且认证未被其他程序更改的情况下恢复原认证。
- 工具意外退出时，已保存的账号仍在钥匙串中；结束 Codex 进程后重新运行工具，选择原账号恢复。
- 支持当前检查过的官方 macOS ChatGPT `26.825.51511` 内置 Codex，或独立安装且文件权限安全的 Codex CLI。优先使用内置组件；CLI-only 环境从 `CODEX_CLI_PATH`、当前 `PATH` 及常见 Homebrew、npm、nvm、Volta、asdf、mise 路径查找。Prism 不自动下载可执行文件。所有登录结果仍须符合默认目录、文件认证及完整 ChatGPT 令牌格式。历史 `Codex Auth` 钥匙串条目会被忽略且不会被修改；默认认证显式使用 `keyring`、`auto`、`ephemeral`、Secrets、未知认证格式、自定义登录策略或不安全文件权限时拒绝操作，绝不自动修改安全策略。
...
- 从普通 Dock 图标或终端启动客户端会使用最后一次替换后的默认认证。Prism 不管理自定义 `CODEX_HOME`、其他应用副本、旧版 ChatGPT Classic 或远程主机；临时 `CODEX_HOME` 只用于隔离的官方登录流程。
...
bash scripts/packageRelease.sh v0.3.6
```
...
归档中的应用位于 `build/Prism.xcarchive/Products/Applications/Prism.app`；最终分发文件位于 `build/release/`，包含 `prism-v0.3.6-macos-arm64.dmg`、`SHA256SUMS.txt` 和发行说明。打包脚本先读取 Xcode 的有效配置，并核对归档中版本、标识、最低系统版本和可执行文件名，拒绝版本不一致的归档。
...
可单独运行 `bash scripts/packageRelease.sh v0.3.6 --check-only`，无需先构建归档。为避免旧附件混入，本地再次打包前需移走已有的 `build/release/`；CI 使用新运行环境。Xcode 普通构建不生成 DMG，分发阶段只生成一次 DMG。
...
```sh
bash scripts/packageRelease.sh v0.3.6 --signed
```
