...
新添加的账号使用登录邮箱命名；认证中没有可用邮箱时使用编号名称。添加操作不会退出当前账号，也不会中断当前任务。首次从当前登录切换到已添加账号时，Prism 会按切换安全流程自动备份离开账号的最新认证，无需单独保存。

### 切换账号

已保存账号显示在菜单顶部，当前账号置顶，以勾选、粗体和不可点击状态标识。团队、Business、Enterprise 和 Edu 工作区账号还会在邮箱后显示“团队”标记；类型来自登录时选定工作区的认证信息，不随额度刷新变化。同一邮箱的个人工作区和团队工作区会按各自的工作区 ID 分开保存和显示。
...
打开“删除账号”子菜单并选择一个邮箱，确认后即可删除对应的本机备份：

- 删除只移除 Prism 钥匙串中的备份，不删除云端账号或默认认证，也不会退出当前登录。
- 确认框中的账号名称可以选择和复制，便于删除前核对。
- 删除后台账号后不能再直接切回，需要通过“添加账号…”重新登录。
- 删除当前账号的备份后，该账号会显示为尚未保存；之后切换账号时，Prism 仍会按安全流程保存离开账号的最新认证。
...
“状态栏额度”子菜单可以关闭额度文字，或选择显示简略额度或全部额度。状态栏使用紧凑的 `5h 79%` 或 `7d 42%` 格式；全部模式恰有两条数据时分两行显示，只显示当前登录账号的额度；默认关闭。所选方式会保存在本机，并在下次启动时继续使用。所选额度尚未取得时，状态栏只显示 Prism 图标，不会把未知额度显示为 0%。

当前已登录但尚未保存的账号也会显示额度，并标为“当前账号（尚未保存）”；必须保存后才能从其他账号切回。VoiceOver 可以读取完整账号名称和额度。

Prism 启动时查询额度。状态栏额度开启时，当前账号约每分钟刷新；关闭时约每 3 分钟刷新，其他账号约每 15 分钟刷新。周期刷新带有少量随机偏移。展开菜单时先显示缓存：当前账号数据超过 1 分钟、其他账号超过 5 分钟时，只刷新达到门槛的账号。接近服务端给出的重置时间时，Prism 会在重置后约 15 秒确认，尚未确认时最多再按分钟尝试 3 次。最多同时查询 3 个账号。额度缓存只保存在内存中，退出 Prism 后清除。

时间已经到达但服务端尚未返回新数据时，界面显示“等待重置确认”，不会自行推断额度已经恢复。请求失败时保留已有数值，显示格式不变，不会用 0% 代替未知值：
...
临时网络或服务失败按账号依次等待 15、30、60 秒，后续最长等待 60 秒，成功后恢复正常周期。展开菜单可提前重试临时失败的账号，每次失败后至少冷却 15 秒；其他失败仍按 5、10、20、30 分钟退避。系统唤醒、屏幕恢复和网络恢复时自动检查，无需手动刷新。服务返回 429 时仍按 `Retry-After` 在 5–60 分钟内退避，恢复事件不能跳过限流或认证失败退避。失败时保留上次数据并显示重试时间；账号加载失败每 30 秒重新检查。
