...
“状态栏额度”子菜单可以关闭额度文字，或选择显示 5 小时、周额度、两者同时显示。状态栏使用紧凑的 `5h 79%`、`7d 42%` 或 `5h 79% · 7d 42%` 格式，只显示当前登录账号的额度；默认关闭。所选方式会保存在本机，并在下次启动时继续使用。所选额度尚未取得时，状态栏只显示 Prism 图标，不会把未知额度显示为 0%。
...
普通失败按账号依次等待 5、10、20、30 分钟，成功后恢复正常周期；服务返回 429 时按 `Retry-After` 在 5–60 分钟内退避。手动触发也遵守退避时间。
