...
临时网络或服务失败按账号依次等待 15、30、60、120、300 秒，成功后恢复正常周期；其他失败仍按 5、10、20、30 分钟退避。系统唤醒、屏幕恢复和网络恢复时自动检查，无需手动刷新。服务返回 429 时仍按 `Retry-After` 在 5–60 分钟内退避，恢复事件不能跳过限流或认证失败退避。失败时保留上次数据并显示重试时间；账号加载失败每 30 秒重新检查。
