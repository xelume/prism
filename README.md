...
3. 选择“添加账号”。工具使用 ChatGPT.app 内置或独立安装的官方 Codex CLI，在隔离的临时 `CODEX_HOME` 中打开浏览器登录；当前默认认证和运行中的任务不变。登录成功后自动校验认证、以邮箱命名并保存到钥匙串，临时目录随后删除。
...
添加或重新登录账号时，浏览器可能记住上一个账号，请核对实际选择。取消、超时或认证格式不受支持时不会修改当前登录或账号备份。官方登录组件本身不提供后台账号的常驻刷新；当前账号由正在使用它的 Codex 自动续期，后台账号失效后需点击重新登录。
